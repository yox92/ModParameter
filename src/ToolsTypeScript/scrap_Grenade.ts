...
import {GrenadeEnum} from "../ListIdItem/GrenadeEnum";
...
async function fetchAmmoData(id: string): Promise<Templates<Ammo>> {
    ...
    const ammoProps = new Ammo({
        ...
        ExplosionStrength: itemData._props.ExplosionStrength,
        MaxExplosionDistance: itemData._props.MaxExplosionDistance,
        FuzeArmTimeSec: Math.round(itemData._props.FuzeArmTimeSec * 1000)
    });
...
async function main() {
    const ids: string[] = EnumUtils.getAllValues(GrenadeEnum)
