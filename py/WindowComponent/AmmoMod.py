...
class AmmoMod:
    ...
    #         self.right_main,text="Reset",command=lambda pname=prop_value: self.reset_slider(pname),width=10)
    ...
    def update_switch(self, name, value, from_save):
        ...
        if name == EnumAmmo.TRACERCOLOR.label:
            label.configure(text="Red" if value else "Green")
        else:
    ...
    def get_entry_value(self, event, prop_value):
        ...
        if isinstance(int_input_text, int):
            if not Utils.is_value_outside_limits_ammo(prop_value,int_input_text):
                self.reset_apply_button()
                self.data_from_json_no_save.update_from_props_json(prop_value, input_text)
            else:
                self.error_number_out_limit()
    ...
    def error_number_out_limit(self):
